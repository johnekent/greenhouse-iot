...
def test_connect_none_on_none(monkeypatch):
    monkeypatch.setattr(MySensor, "_connect", mock_none_connection)
    s = MySensor()
    assert not s.connection

def test_connect_none_on_fail(monkeypatch):
    monkeypatch.setattr(MySensor, "_connect", mock_raises_connection)
    s = MySensor()
    assert not s.connection

def test_connect_not_none_on_good_connection(monkeypatch):
    monkeypatch.setattr(MySensor, "_connect", mock_good_connection)
    s = MySensor()
    assert s.connection

def test_read_returns_on_good_connection(monkeypatch):
    monkeypatch.setattr(MySensor, "_connect", mock_good_connection)
    monkeypatch.setattr(MySensor, "_read", mock_read_good_message)
    s = MySensor()
    assert s.read()
...
def test_connect_returns_none_on_bad_and_none_first_read_but_then_read_works_after_good_and_then_read_raises_sets_connect_to_none(monkeypatch):
    ...
    monkeypatch.setattr(MySensor, "_read", mock_raises_read)
